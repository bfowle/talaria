...
use talaria_core::{DatabaseSource, DatabaseSourceInfo, NCBIDatabase, UniProtDatabase};
use super::taxonomy_manager::{TaxonomyManager, VersionDecision};
...
impl DatabaseManager {
    ...
    pub fn get_current_version_info(&self, source: &DatabaseSource) -> Result<VersionInfo> {
        use crate::core::versioning::version_detector::DatabaseVersion;
    ...
    async fn handle_incremental_update(
        &mut self,
        new_manifest: talaria_sequoia::Manifest,
        progress_callback: impl Fn(&str) + Send + Sync,
    ) -> Result<DownloadResult> {
        use talaria_sequoia::operations::state::{OperationType, SourceInfo};
        ...
        let mut temporal_index = talaria_sequoia::TemporalIndex::load(&temporal_path)?;
    ...
    fn create_or_update_taxonomy_manifest(
        ...
        let mut manifest: TaxonomyManifest = if manifest_path.exists() {
            ...
            if alt_path.exists() {
                ...
                TaxonomyManifest {
                    ...
                    updated_at: Utc::now(),
                    expected_components:
                        crate::core::database::taxonomy_manager::TaxonomyManager::default_components(),
        ...
        // Detect file format
        let file_format =
            crate::core::database::taxonomy_manager::TaxonomyManager::detect_file_format(file_path)?;
    ...
    pub fn chunk_sequences_direct(
        ...
        let sequences_path = self.base_path.join("sequences");
        let sequence_storage = SequenceStorage::new(&sequences_path)?;

        // Create chunker with canonical storage
        // Convert DatabaseSource enum to DatabaseSourceInfo struct for internal use
        let database_source: DatabaseSourceInfo = source.into();
    ...
    fn save_database_manifest(
        &mut self,
        manifests_with_hashes: Vec<(talaria_sequoia::ChunkManifest, talaria_sequoia::SHA256Hash)>,
        source: &DatabaseSource,
    ) -> Result<()> {
        use talaria_sequoia::{ManifestMetadata, TemporalManifest, BiTemporalCoordinate};
        use chrono::Utc;
        ...
        let manifest = TemporalManifest {
            ...
            taxonomy_root: talaria_sequoia::SHA256Hash::zero(),
            sequence_root: talaria_sequoia::SHA256Hash::zero(),
            chunk_merkle_tree: None,
            taxonomy_manifest_hash: talaria_sequoia::SHA256Hash::zero(),
    ...
    fn store_chunk_manifests(
        &mut self,
        manifests: Vec<talaria_sequoia::ChunkManifest>,
        _source: &DatabaseSource,
    ) -> Result<Vec<(talaria_sequoia::ChunkManifest, talaria_sequoia::SHA256Hash)>> {
        use talaria_sequoia::ManifestMetadata;
    ...
    fn create_version_metadata(
        ...
        use crate::core::versioning::version_detector::{DatabaseVersion, VersionDetector};
    ...
    pub fn get_taxonomy_mapping_from_manifest(
        &self,
        source: &DatabaseSource,
    ) -> Result<std::collections::HashMap<String, talaria_sequoia::TaxonId>> {
        use std::collections::HashMap;
    ...
    fn load_taxonomy_mapping(
        ...
        let pb = crate::cli::progress::create_spinner("Parsing taxonomy mappings");
                ...
                for line_result in reader.lines() {
                    ...
                    if parts.len() >= 3 && parts[1] == "NCBI_TaxID" {
                        if let Ok(taxid) = parts[2].parse::<u32>() {
                            mappings.insert(parts[0].to_string(), talaria_sequoia::TaxonId(taxid));
                        }
                    }
                }
            }
                ...
                for line_result in lines {
                    ...
                    if parts.len() >= 2 {
                        if let Ok(taxid) = parts[1].parse::<u32>() {
                            ...
                            mappings.insert(accession.clone(), talaria_sequoia::TaxonId(taxid));

                            // Also store without version suffix
                            if let Some(dot_pos) = accession.rfind('.') {
                                mappings.insert(
                                    accession[..dot_pos].to_string(),
                                    talaria_sequoia::TaxonId(taxid),
                                );
                            }
                        }
                    }
    ...
    fn find_latest_version_dir(&self, dataset_path: &Path) -> Result<Option<PathBuf>> {
        use crate::core::versioning::version_detector::is_timestamp_format;
    ...
    fn read_manifest(&self, path: &Path) -> Result<talaria_sequoia::TemporalManifest> {
        ...
        if path.extension().and_then(|s| s.to_str()) == Some("tal") {
                ...
                && &content[..TALARIA_MAGIC.len()] == TALARIA_MAGIC
            {
                ...
                let manifest: talaria_sequoia::TemporalManifest =
        ...
        let manifest: talaria_sequoia::TemporalManifest = serde_json::from_slice(&content)?;
        Ok(manifest)
    }
    ...
    pub fn init_temporal_for_existing(&mut self) -> Result<()> {
        ...
        if root_manifest.exists() {
            if let Ok(content) = std::fs::read_to_string(&root_manifest) {
                if let Ok(manifest) =
                    serde_json::from_str::<talaria_sequoia::TemporalManifest>(&content)
                {
    ...
    pub fn list_resumable_operations(&self) -> Result<Vec<(String, talaria_sequoia::ProcessingState)>> {
    ...
    pub fn get_storage(&self) -> &talaria_sequoia::SEQUOIAStorage {
    ...
    pub fn assemble_database(&self, source: &DatabaseSource, output_path: &Path) -> Result<()> {
        ...
        let assembler = talaria_sequoia::FastaAssembler::new(&self.repository.storage);
...
impl DatabaseManager {
    ...
    pub fn verify_chunk_proof(&self, chunk_hash: &talaria_sequoia::SHA256Hash) -> Result<bool> {
    ...
    pub fn get_manifest(
        ...
        let db_ref = crate::core::database::database_ref::parse_database_reference(database_name)?;
    ...
    pub fn load_manifest(
        &self,
        hash: &talaria_sequoia::SHA256Hash,
    ) -> Result<talaria_sequoia::ChunkManifest> {
        ...
        if let Ok(manifest) = serde_json::from_slice::<talaria_sequoia::ChunkManifest>(&chunk_data) {
            return Ok(manifest);
        }
    ...
    pub fn load_sequences_from_manifest(
        &self,
        manifest: &talaria_sequoia::ChunkManifest,
        filter: Option<&str>,
        limit: usize,
    ...
    pub fn load_taxonomy_mappings(
        ...
        let source = crate::download::parse_database_source(database_name)
...
pub struct TemporalSequenceRecord {
    ...
    pub chunk_hash: talaria_sequoia::SHA256Hash,
}
...
mod tests {
    ...
    /// Helper to create a test SEQUOIA manager with temp directory
    ...
    fn create_fake_manifest() -> talaria_sequoia::TemporalManifest {
    ...
    fn test_multiple_database_manifests_coexist() {
        ...
        let sp_content: talaria_sequoia::TemporalManifest =
            serde_json::from_str(&fs::read_to_string(&swissprot_path).unwrap()).unwrap();
        let tr_content: talaria_sequoia::TemporalManifest =
