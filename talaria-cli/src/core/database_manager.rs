...
use talaria_sequoia::{
    SEQUOIARepository, ChunkingStrategy, SHA256Hash, TaxonomicChunker, TaxonomyAwareChunk,
};
...
impl DatabaseManager {
    ...
    async fn handle_incremental_update(
        ...
        use talaria_sequoia::{OperationType, SourceInfo};
        ...
        let mut temporal_index = talaria_sequoia::temporal::TemporalIndex::load(&temporal_path)?;
    ...
    fn create_new_taxonomy_version_internal(&self, copy_forward: bool) -> Result<PathBuf> {
        ...
        let new_version = talaria_core::paths::generate_utc_timestamp();
        println!("Creating new taxonomy version: {} (UTC)", new_version);

        let new_version_dir = talaria_core::paths::talaria_taxonomy_version_dir(&new_version);
        std::fs::create_dir_all(&new_version_dir)?;

        // Copy existing files from current version if requested
        if copy_forward {
            let current_dir = talaria_core::paths::talaria_taxonomy_current_dir();
        ...
        let current_link = talaria_core::paths::talaria_taxonomy_versions_dir().join("current");
    ...
    fn store_taxonomy_mapping_file(
        ...
        let (taxonomy_dir, version) = match version_decision {
            ...
            VersionDecision::AppendToCurrent => {
                ...
                let current = talaria_core::paths::talaria_taxonomy_current_dir();
                if !current.exists() {
    ...
    pub fn chunk_sequences_direct(
        ...
        // Store chunks in SEQUOIA (rest of the method remains the same)
        self.store_chunks_in_sequoia(chunks, source)?;

        Ok(())
    }

    /// Store chunks in SEQUOIA repository
    ...
    fn create_versioned_manifest_path(
        ...
        let version = talaria_core::paths::generate_utc_timestamp();
    ...
    fn get_versions_dir(&self, source: &DatabaseSource) -> PathBuf {
        ...
        if matches!(
            ...
            return talaria_core::paths::talaria_taxonomy_versions_dir();
        }
    ...
    pub fn get_taxonomy_mapping_from_manifest(
        ...
        for chunk_meta in manifest.chunk_index.iter() {
            ...
            let sequences = talaria_bio::fasta::parse_fasta_from_bytes(&chunk_data)?;
    ...
    fn load_taxonomy_mapping(
        ...
        let mappings_dir = talaria_core::paths::talaria_taxonomy_current_dir().join("mappings");
...
impl DatabaseManager {
    ...
    ) -> Result<talaria_sequoia::types::TemporalManifest> {
    ...
    pub fn verify_chunk_proof(&self, chunk_hash: &talaria_sequoia::types::SHA256Hash) -> Result<bool> {
    ...
    pub fn get_manifest(
        ...
        let db_ref = crate::utils::database_ref::parse_database_reference(database_name)?;
    ...
    pub fn load_chunk(
    ...
    ) -> Result<talaria_sequoia::types::TaxonomyAwareChunk> {
        ...
        if chunk_data.len() > 4 && &chunk_data[0..4] == TALARIA_MAGIC {
            let chunk: talaria_sequoia::types::TaxonomyAwareChunk =
