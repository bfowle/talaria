...
pub fn generate_text_report(result: &ComparisonResult, options: &ReportOptions) -> Result<String> {
    ...
    stats_table.add_row(vec![
        ...
        Cell::new(format_number(stats.old_avg_length)),
        Cell::new(format_number(stats.new_avg_length)),
            ...
            for mod_seq in result.modified.iter().take(10) {
                ...
                let changes_str = mod_seq
                    ...
                    .map(|c| match c {
                        ...
                        SequenceChange::Truncated(n) => format!("Trunc -{}", n),
                        SequenceChange::Mutations(n) => format!("{} mut", n),
                    })
                    .collect::<Vec<_>>()
                    .join(", ");
