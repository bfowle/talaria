...
use talaria_bio::taxonomy::{TaxonomyDiscrepancy, TaxonomyEnrichable, TaxonomyResolver};
...
impl TaxonomicChunker {
    ...
    fn create_chunk(
        ...
        let taxonomy_version = if let Ok(tax_mgr) =
            crate::taxonomy::TaxonomyManager::new(&paths::talaria_databases_dir())
        {
        ...
        for (taxon_id, chunks) in special_taxa_chunks {
            ...
            match special_taxon.strategy {
                ...
                ChunkStrategy::GroupWithSiblings => {
                    ...
                    use talaria_core::paths;
                    let taxonomy = crate::taxonomy::TaxonomyManager::new(
