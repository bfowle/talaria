...
use crate::types::*;
use talaria_bio::delta_encoder::{DeltaEncoder, DeltaRecord};
use anyhow::Result;
use chrono::Utc;
...
impl DeltaGenerator {
    ...
    pub fn generate_delta_chunks(
        ...
        if !full_sequences.is_empty() {
            // These would be stored as regular SEQUOIA chunks, not delta chunks
    ...
    /// Create a SEQUOIA delta chunk from delta records
    fn create_delta_chunk(
        ...
        // Convert to SEQUOIA delta operations
        let mut delta_operations = Vec::new();
        for record in &delta_records {
            delta_operations.push(self.convert_to_sequoia_operation(record)?);
        }
        ...
        // Create the delta chunk using the new SEQUOIA types
        ...
        // Use the SEQUOIA DeltaChunk type that's defined in types.rs
        let delta_chunk = DeltaChunk {
            content_hash: content_hash.clone(),
            reference_hash: reference_chunk_hash,
            chunk_type,
            taxonomy_version: SHA256Hash::zero(), // Will be set by SEQUOIA
            taxon_ids,
    ...
    /// Convert a delta record to SEQUOIA delta operation
    fn convert_to_sequoia_operation(&self, record: &DeltaRecord) -> Result<DeltaOperation> {
