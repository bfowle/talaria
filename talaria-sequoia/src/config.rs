/// Configuration system for SEQUOIA storage backends
...
impl SequoiaConfig {
    ...
    pub fn load() -> Result<Self> {
        ...
        if let Ok(config_path) = env::var("TALARIA_SEQUOIA_CONFIG") {
            config = Self::from_file(Path::new(&config_path))?;
        } else if let Ok(home) = env::var("TALARIA_HOME") {
            let default_config = PathBuf::from(home).join("sequoia.toml");
...
pub fn generate_default_config(path: &Path) -> Result<()> {
    let config = SequoiaConfig::default();
    config.save(path)?;
...
mod tests {
    ...
    fn test_config_serialization() {
        ...
        let deserialized: SequoiaConfig = toml::from_str(&serialized).unwrap();
    ...
    fn test_config_file_loading() {
        ...
        let loaded = SequoiaConfig::from_file(temp_file.path()).unwrap();
    ...
    fn test_env_overrides() {
        ...
        let mut config = SequoiaConfig::default();
