...
impl ValidatorImpl {
    ...
    pub fn calculate_metrics(
    ...
    ) -> Result<ValidationMetrics, talaria_core::error::TalariaError> {
        ...
        let original_taxa: HashSet<u32> = original.iter().filter_map(|s| s.taxon_id).collect();
    ...
    pub fn compare_alignments<P: AsRef<Path>>(
        ...
        original_results: P,
        reduced_results: P,
    ) -> Result<AlignmentMetrics, talaria_core::error::TalariaError> {
    ...
    fn parse_m8<P: AsRef<Path>>(
        ...
        for line in reader.lines() {
            ...
            if parts.len() >= 2 {
                ...
                results.entry(query).or_default().push(subject);
            }
        }
