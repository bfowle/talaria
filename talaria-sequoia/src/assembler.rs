...
use crate::types::*;
use crate::verifier::SEQUOIAVerifier;
...
impl<'a> FastaAssembler<'a> {
    ...
    fn extract_sequences_from_chunk(&self, hash: &SHA256Hash) -> Result<Vec<Sequence>> {
        ...
        if let Ok(chunk) = serde_json::from_slice::<crate::TaxonomyAwareChunk>(&chunk_data) {
    ...
    fn stream_chunk_to_writer<W: Write>(&self, hash: &SHA256Hash, writer: &mut W) -> Result<usize> {
        ...
        let taxon_id = if let Ok(chunk) =
            serde_json::from_slice::<crate::TaxonomyAwareChunk>(&chunk_data)
        {
        ...
        for seq in &sequences {
            ...
            let tid = taxon_id.or(seq.taxon_id);
