...
#[test]
#[ignore]
fn test_sequoia_storage_with_chunks() {
    let temp_dir = TempDir::new().unwrap();
    let sequoia = SequoiaStorage::new(temp_dir.path()).unwrap();

    println!("Testing SEQUOIA storage with chunk operations...");

    // Store multiple chunks
    let mut chunk_hashes = Vec::new();
    for i in 0..1000 {
        let data = format!("Test chunk data {}", i).into_bytes();
        let hash = sequoia.store_chunk(&data, true).unwrap(); // With compression
    ...
    for hash in &chunk_hashes[..10] {
        // Test first 10
        let data = sequoia.get_chunk(hash).unwrap();
        assert!(!data.is_empty());
    }

    // Test enumeration
    let all_chunks = sequoia.enumerate_chunks();
    assert_eq!(all_chunks.len(), 1000);

    // Test statistics
    let stats = sequoia.get_stats();
