...
fn test_taxonomy_query_performance() {
    ...
    println!("  Found {} bacterial sequences in {:?}", bacterial_sequences.len(), cold_time);
    assert!(bacterial_sequences.len() >= 400_000, "Should find all bacterial sequences");
    assert!(cold_time.as_secs() < 1, "Cold query should complete in <1 second");
    ...
    println!("  Found {} bacterial sequences in {:?}", bacterial_sequences_cached.len(), warm_time);
    assert_eq!(bacterial_sequences.len(), bacterial_sequences_cached.len());
    assert!(warm_time.as_millis() < 100, "Cached query should complete in <100ms");
    ...
    println!("  Matched {} sequences in {:?}", filtered_count, complex_time);
    assert!(complex_time.as_millis() < 10, "Filter evaluation should be <10ms");
    ...
    println!("  100K lookups in {:?} ({:.0} ops/sec)", bloom_time, ops_per_second);
    ...
    println!("✓ Bloom filter: {:.0} ops/sec (target: >1M)", ops_per_second);
}
...
fn test_taxonomy_filter_correctness() {
    ...
    let test_cases = vec![
        ...
        ("Bacteria AND Escherichia", vec![TaxonId(2), TaxonId(561)], true),
        ("Bacteria AND Escherichia", vec![TaxonId(2)], false),
        ("Bacteria AND NOT Escherichia", vec![TaxonId(2)], true),
        ("Bacteria AND NOT Escherichia", vec![TaxonId(2), TaxonId(561)], false),
        ("(Bacteria OR Archaea) AND NOT Viruses", vec![TaxonId(2)], true),
        ("(Bacteria OR Archaea) AND NOT Viruses", vec![TaxonId(2157)], true),
        ("(Bacteria OR Archaea) AND NOT Viruses", vec![TaxonId(10239)], false),
    ];
