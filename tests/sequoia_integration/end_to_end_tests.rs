...
use talaria_sequoia::temporal::core::TemporalIndex;
...
fn test_complete_reduction_workflow() -> Result<()> {
    ...
    for seq in &sequences {
        ...
        for chunk_data in data.chunks(chunk_size) {
            let hash = storage.store_chunk(chunk_data, talaria_sequoia::storage::ChunkFormat::Zstd)?;
            manifest.add_chunk(hash, chunk_data.len(), talaria_sequoia::storage::ChunkFormat::Zstd);
            chunk_count += 1;
        }
    }
...
fn test_incremental_update_workflow() -> Result<()> {
    ...
    for seq in &initial {
        let data = seq.to_fasta_string().into_bytes();
        let hash = storage.store_chunk(&data, talaria_sequoia::storage::ChunkFormat::Raw)?;
...
fn test_taxonomic_chunking_workflow() -> Result<()> {
    ...
    for (taxid, seqs) in tax_groups {
        ...
        let hash = storage.store_chunk(&group_data, talaria_sequoia::storage::ChunkFormat::Zstd)?;
        manifest.add_chunk(hash, group_data.len(), talaria_sequoia::storage::ChunkFormat::Zstd);
...
fn test_compression_effectiveness() -> Result<()> {
    ...
    let raw_hash = storage.store_chunk(&repetitive_data, talaria_sequoia::storage::ChunkFormat::Raw)?;
    let zstd_hash = storage.store_chunk(&repetitive_data, talaria_sequoia::storage::ChunkFormat::Zstd)?;
    let raw_random = storage.store_chunk(&random_data, talaria_sequoia::storage::ChunkFormat::Raw)?;
    let zstd_random = storage.store_chunk(&random_data, talaria_sequoia::storage::ChunkFormat::Zstd)?;
...
fn test_chunk_deduplication_across_databases() -> Result<()> {
    ...
    let shared_hash1 = storage.store_chunk(&shared_data, talaria_sequoia::storage::ChunkFormat::Raw)?;
    let unique_hash1 = storage.store_chunk(&unique_data1, talaria_sequoia::storage::ChunkFormat::Raw)?;
    manifest1.add_chunk(shared_hash1, shared_data.len(), talaria_sequoia::storage::ChunkFormat::Raw);
    manifest1.add_chunk(unique_hash1, unique_data1.len(), talaria_sequoia::storage::ChunkFormat::Raw);
    
    // Store in second database
    let shared_hash2 = storage.store_chunk(&shared_data, talaria_sequoia::storage::ChunkFormat::Raw)?;
    let unique_hash2 = storage.store_chunk(&unique_data2, talaria_sequoia::storage::ChunkFormat::Raw)?;
    manifest2.add_chunk(shared_hash2, shared_data.len(), talaria_sequoia::storage::ChunkFormat::Raw);
    manifest2.add_chunk(unique_hash2, unique_data2.len(), talaria_sequoia::storage::ChunkFormat::Raw);
...
fn test_recovery_from_partial_manifest() -> Result<()> {
    ...
    for hash in &chunk_hashes[..10] {
        partial_manifest.add_chunk(*hash, 1000, talaria_sequoia::storage::ChunkFormat::Raw);
    }
...
fn test_large_scale_storage_performance() -> Result<()> {
    ...
    for i in 0..sequence_count {
        ...
        let hash = storage.store_chunk(&data, talaria_sequoia::storage::ChunkFormat::Zstd)?;
        manifest.add_chunk(hash, data.len(), talaria_sequoia::storage::ChunkFormat::Zstd);
...
fn test_concurrent_access_safety() -> Result<()> {
    ...
    let initial_hash = storage.store_chunk(&initial_data, talaria_sequoia::storage::ChunkFormat::Raw)?;
