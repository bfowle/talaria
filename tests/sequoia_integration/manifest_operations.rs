/// Integration test to verify SEQUOIA manifest path fix
...
mod tests {
    ...
    fn test_manifest_saved_to_correct_path() {
        ...
        let sequoia_dir = temp_dir.path().join("databases");

        // Expected path for SwissProt manifest
        let expected_path = sequoia_dir.join("manifests").join("uniprot-swissprot.json");
        ...
        let old_path = sequoia_dir.join("manifest.json");
        assert!(
            !old_path.exists(),
    ...
    fn test_multiple_database_manifests_coexist() {
        let temp_dir = TempDir::new().unwrap();
        let sequoia_dir = temp_dir.path().join("databases");
        let manifests_dir = sequoia_dir.join("manifests");
    ...
    fn test_old_manifest_migration() {
        let temp_dir = TempDir::new().unwrap();
        let sequoia_dir = temp_dir.path().join("databases");
        fs::create_dir_all(&sequoia_dir).unwrap();
        ...
        let new_path = sequoia_dir.join("manifests").join("uniprot-swissprot.json");
    ...
    fn test_manifest_directory_structure() {
        ...
        // Expected structure
        let manifests_dir = sequoia_dir.join("manifests");
        let chunks_dir = sequoia_dir.join("chunks");
        let taxonomy_dir = sequoia_dir.join("taxonomy");
