...
fn test_both_algorithms_end_to_end() {
    ...
    assert!(!result_sp.references.is_empty(), "SinglePass should select references");
    assert!(!result_sm.references.is_empty(), "SimilarityMatrix should select references");
    ...
    assert!(result_sp.references.iter().any(|r| r.id == "seq1"),
            "SinglePass should select longest sequence");
    assert!(result_sm.references.iter().any(|r| r.id == "seq1"),
            "SimilarityMatrix should select longest sequence");

    // Too short sequences should be discarded
    assert!(result_sp.discarded.contains("seq7"),
            "SinglePass should discard too-short sequences");
    assert!(result_sm.discarded.contains("seq7"),
...
fn test_algorithm_with_reducer_pipeline() {
    ...
    let result_sp = reducer_sp.reduce(
        sequences.clone(),
        0.5,
    ...
    let result_sm = reducer_sm.reduce(
        sequences.clone(),
        0.5,
        TargetAligner::Generic,
    );
    ...
    // Should complete without error
    assert!(result_sm.is_ok(), "SimilarityMatrix reduction should succeed");
...
fn test_algorithm_consistency() {
    ...
    assert_eq!(result1.references.len(), result2.references.len(),
...
fn test_algorithm_invariants() {
    ...
    for algorithm in [SelectionAlgorithm::SinglePass, SelectionAlgorithm::SimilarityMatrix] {
        ...
        let all_ids: Vec<_> = result.references.iter().map(|r| &r.id)
        ...
        let ref_ids: std::collections::HashSet<_> = result.references.iter().map(|r| &r.id).collect();
        for children in result.children.values() {
            for child in children {
                assert!(!ref_ids.contains(&child),
