...
fn setup_test_casg() -> (TempDir, CASGRepository) {
...
#[test]
fn test_casg_to_reduce_workflow() {
    ...
    let chunks = chunker.chunk_sequences(sequences.clone()).unwrap();

    for chunk in &chunks {
        repo.storage.store_taxonomy_chunk(chunk).unwrap();
    }

    // Step 2: Assemble FASTA from CASG
    ...
    // Step 4: Verify we can expand reduced sequences using CASG chunks
    // (This would require implementing delta expansion with CASG)
}
...
fn test_incremental_update_simulation() {
    ...
    let chunker = TaxonomicChunker::new(ChunkingStrategy::default());
    let v1_chunks = chunker.chunk_sequences(v1_sequences.clone()).unwrap();
    ...
    let v2_chunks = chunker.chunk_sequences(v2_sequences.clone()).unwrap();
...
fn test_version_tracking_through_workflow() {
    // use talaria::casg::{VersionIdentifier, VersionInfo};
