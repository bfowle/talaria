...
use talaria_bio::sequence::{Sequence, SequenceType};
use talaria_core::reducer::Reducer;
use talaria_core::reference_selector::ReferenceSelectorImpl;
...
fn test_delta_encoding() {
    use talaria_core::delta_encoder::{DeltaEncoder, DeltaReconstructor};
...
fn test_sequence_reduction() {
    ...
    let (reduced_sequences, _deltas, _original_count) = reducer
        .reduce(sequences, 0.5, TargetAligner::Generic)
        .unwrap();
...
fn test_database_config() {
    use talaria_download::get_database_configs;
