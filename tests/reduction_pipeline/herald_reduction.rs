...
/// Test that HERALD assembly from manifest works
#[test]
fn test_herald_assembly_from_manifest() {
    let temp_dir = TempDir::new().unwrap();
    std::env::set_var("TALARIA_HOME", temp_dir.path());
...
fn test_reduction_manifest_creation() {
    use talaria_herald::reduction::{ReductionManifest, ReductionParameters};
...
fn test_reduction_creates_profile_not_database() {
    ...
    // Since we can't access the repository directly, we need to use HeraldRepository directly
    // but we need to open the exact same path to share the storage
    let herald =
        HeraldRepository::open(&db_path).unwrap_or_else(|_| HeraldRepository::init(&db_path).unwrap());
...
mod integration_tests {
    ...
    #[test]
    #[ignore] // Requires full HERALD setup
