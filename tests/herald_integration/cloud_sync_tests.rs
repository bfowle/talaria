...
mod cloud_tests {
    ...
    use talaria_herald::cloud::sync::CloudSync;
    use talaria_herald::cloud::s3::S3Backend;
    ...
    fn test_s3_sync_workflow() -> Result<()> {
        ...
        let mut local_storage = HeraldStorage::new(local_path.clone())?;
        let mut manifest = Manifest::new("cloud_db".to_string(), "1.0.0".to_string());
        
        // Store test data
        let test_data = "TEST_CLOUD_DATA".repeat(100).into_bytes();
        let hash = local_storage.store_chunk(&test_data, talaria_herald::storage::ChunkFormat::Raw)?;
        manifest.add_chunk(hash, test_data.len(), talaria_herald::storage::ChunkFormat::Raw);
        ...
        // Verify data
        let remote_storage = HeraldStorage::new(remote_path)?;
...
mod mock_cloud_tests {
    ...
    fn test_mock_cloud_push_pull() -> Result<()> {
        ...
        let mut storage1 = HeraldStorage::new(local1.clone())?;
        ...
        let hash1 = storage1.store_chunk(&data1, talaria_herald::storage::ChunkFormat::Raw)?;
        let hash2 = storage1.store_chunk(&data2, talaria_herald::storage::ChunkFormat::Zstd)?;
        
        manifest.add_chunk(hash1, data1.len(), talaria_herald::storage::ChunkFormat::Raw);
        manifest.add_chunk(hash2, data2.len(), talaria_herald::storage::ChunkFormat::Zstd);
        ...
        let storage2 = HeraldStorage::new(local2.clone())?;
    ...
    fn test_incremental_sync() -> Result<()> {
        ...
        let mut storage = HeraldStorage::new(local.clone())?;
        let mut manifest = Manifest::new("incremental_sync_db".to_string(), "1.0.0".to_string());
        
        // Initial data
        let data1 = "INITIAL".repeat(100).into_bytes();
        let hash1 = storage.store_chunk(&data1, talaria_herald::storage::ChunkFormat::Raw)?;
        manifest.add_chunk(hash1, data1.len(), talaria_herald::storage::ChunkFormat::Raw);
        ...
        let hash2 = storage.store_chunk(&data2, talaria_herald::storage::ChunkFormat::Raw)?;
        manifest.add_chunk(hash2, data2.len(), talaria_herald::storage::ChunkFormat::Raw);
        manifest.save(&local.join("manifest.json"))?;
        ...
        let storage3 = HeraldStorage::new(local3)?;
    ...
    fn test_conflict_resolution() -> Result<()> {
        ...
        let mut storage2 = HeraldStorage::new(local2.clone())?;
        ...
        let hash1 = storage1.store_chunk(&data1, talaria_herald::storage::ChunkFormat::Raw)?;
        manifest1.add_chunk(hash1, data1.len(), talaria_herald::storage::ChunkFormat::Raw);
        manifest1.save(&local1.join("manifest.json"))?;
        
        let data2 = "LOCAL2_DATA".repeat(50).into_bytes();
        let hash2 = storage2.store_chunk(&data2, talaria_herald::storage::ChunkFormat::Raw)?;
        manifest2.add_chunk(hash2, data2.len(), talaria_herald::storage::ChunkFormat::Raw);
    ...
    fn test_partial_sync_recovery() -> Result<()> {
        ...
        let partial_storage = HeraldStorage::new(partial)?;
