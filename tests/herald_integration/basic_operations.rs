...
use talaria_herald::{
    HeraldRepository, ChunkMetadata, ChunkingStrategy, FastaAssembler, SHA256Hash, TaxonId,
...
fn setup_test_herald() -> (TempDir, HeraldRepository) {
    let temp_dir = TempDir::new().unwrap();
    let repo = HeraldRepository::init(temp_dir.path()).unwrap();
    (temp_dir, repo)
}
...
#[test]
fn test_herald_to_reduce_workflow() {
    let (_temp_dir, repo) = setup_test_herald();
    let sequences = create_test_sequences();

    // Step 1: Chunk sequences and store in HERALD
    ...
    // Step 2: Assemble FASTA from HERALD
    ...
    // Step 4: Verify we can expand reduced sequences using HERALD chunks
    // (This would require implementing delta expansion with HERALD)
}
...
fn test_version_tracking_through_workflow() {
    // use talaria_herald::{VersionIdentifier, VersionInfo};
