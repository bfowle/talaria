...
use std::fs;
use talaria_herald::{
    HeraldRepository, HeraldStorage, FastaAssembler, MerkleDAG, SHA256Hash, TaxonId,
...
fn setup_test_env() -> (TempDir, HeraldRepository) {
...
fn test_missing_manifest_handling() {
    ...
    let repo_result = HeraldRepository::open(temp_dir.path());
...
fn test_storage_failure_handling() {
    ...
    let storage_result = HeraldStorage::new(&storage_path);
...
fn test_merkle_proof_tampering() {
    use talaria_herald::ChunkMetadata;
