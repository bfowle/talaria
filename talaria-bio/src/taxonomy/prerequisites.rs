...
impl TaxonomyPrerequisites {
    pub fn new() -> Self {
        ...
        Self::with_taxonomy_dir(taxonomy_dir)
    }

    /// Create with a specific taxonomy directory (useful for testing)
    pub fn with_taxonomy_dir(taxonomy_dir: PathBuf) -> Self {
...
pub async fn download_taxonomy_database(database: &str) -> Result<()> {
    ...
    let mut args = DownloadArgs::default_with_database(format!("ncbi/{}", database));
    args.output = paths::talaria_databases_dir();
    args.resume = true;
...
mod tests {
    ...
    fn test_taxonomy_status_check() {
        ...
        // Use the test-specific constructor to avoid environment variable race conditions
        let taxonomy_dir = temp_dir.path().join("taxonomy").join("current");
        let prereqs = TaxonomyPrerequisites::with_taxonomy_dir(taxonomy_dir);
        let status = prereqs.check_status();

        // Should have no files initially
        assert!(!status.has_any, "Expected no files, but has_any is true");
        assert!(!status.has_ncbi_protein);
        assert!(!status.has_uniprot);
        assert_eq!(status.present_files.len(), 0, "Expected 0 present files, got {}", status.present_files.len());
        assert_eq!(status.missing_files.len(), 4, "Expected 4 missing files, got {}", status.missing_files.len()); // All files missing
    }
    ...
    fn test_has_accession_mapping() {
        ...
        // Use the test-specific constructor
        let prereqs = TaxonomyPrerequisites::with_taxonomy_dir(tax_version_dir);
