...
// Import TaxonomyDataSource from talaria-core and rename for compatibility
pub use talaria_core::types::TaxonomyDataSource as TaxonomySource;
...
impl TaxonomySources {
    ...
    pub fn all_sources(&self) -> Vec<(TaxonomySource, u32)> {
        ...
        if let Some(id) = self.api_provided {
            sources.push((TaxonomyDataSource::Api, id));
        }
        if let Some(id) = self.user_specified {
            sources.push((TaxonomyDataSource::User, id));
        }
        if let Some(id) = self.mapping_lookup {
            sources.push((TaxonomyDataSource::Accession2Taxid, id));
        }
        if let Some(id) = self.header_parsed {
            sources.push((TaxonomyDataSource::Header, id));
        }
        if let Some(id) = self.chunk_context {
            sources.push((TaxonomyDataSource::Inherited, id));
        }
        ...
        match self {
            ...
            TaxonomyResolution::Unanimous { sources, .. } => {
                    ...
                    .any(|(s, _)| matches!(s, TaxonomySource::Api))
                {
                    ...
                    .iter()
                    .any(|(s, _)| matches!(s, TaxonomySource::Accession2Taxid))
                {
...
pub trait SequenceProvider {
    ...
    fn fetch_sequences(&self) -> Result<Vec<crate::sequence::Sequence>>;
