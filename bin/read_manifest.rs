use std::path::Path;
use talaria::sequoia::Manifest;

fn main() {
    let manifest_path = Path::new(
        "/home/brett/.talaria/databases/versions/custom/cholera/20250918_040507/manifest.tal",
    );
            ...
            if let Some(data) = manifest.get_data() {
                ...
                for (i, chunk) in data.chunk_index.iter().enumerate() {
                    ...
                    if !chunk.taxon_ids.is_empty() {
                        println!(
                            "  First few taxon IDs: {:?}",
                            &chunk.taxon_ids[..chunk.taxon_ids.len().min(5)]
                        );
                    }
                }
            }
