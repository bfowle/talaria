...
pub fn generate_json_report(result: &ComparisonResult, _options: &ReportOptions) -> Result<String> {
    let report = json!({
        ...
        "changes": {
            ...
            "modified": result.modified.iter().map(|mod_seq| {
                let old = mod_seq.old.as_ref();
                let new = mod_seq.new.as_ref();
                json!({
                    "id": old.map(|o| &o.id),
                    "old_length": old.map(|o| o.length),
                    "new_length": new.map(|n| n.length),
                    "similarity": mod_seq.similarity,
                    "changes": mod_seq.changes.as_ref().map(|c| c.iter().map(|s| s.as_str()).collect::<Vec<_>>()),
                })
            }).collect::<Vec<_>>(),
