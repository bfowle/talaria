...
pub enum SelectionAlgorithm {
    ...
    Hybrid,
    /// Graph centrality-based algorithm (CASG architecture specification)
...
impl ReferenceSelectorImpl {
    ...
    pub fn select_references_with_lambda(
        ...
        use crate::cli::output::{
            action, info, section_header, subsection_header, success, warning,
        };
        ...
        let alignments = if self.all_vs_all {
            ...
            for (taxon_id, group_sequences) in taxon_groups.iter() {
                ...
                subsection_header(&format!(
                    "Taxon {} ({} sequences)",
                    taxon_id,
                    format_number(group_sequences.len())
                ));
        ...
        match self.selection_algorithm {
            ...
            SelectionAlgorithm::GraphCentrality => {
                // Use graph centrality approach from CASG architecture
                use crate::cli::output::info;
                info("Using graph centrality-based selection (CASG 5-dimensional)");
    ...
    fn select_with_single_pass(
        ...
        pb2.finish_and_clear();
        println!(
            "Selected {} references, {:.1}% coverage",
            references.len(),
            final_coverage * 100.0
        );
    ...
    /// Graph centrality-based selection algorithm from CASG architecture
    ...
    fn select_with_graph_centrality(
        ...
        use crate::core::reference_selector_optimized::{
            AlignmentCache, OptimizedReferenceSelector,
        };

        section_header("Graph Centrality Algorithm (CASG)");
        ...
        let mut mock_aligner = crate::tools::MockAligner::new();
