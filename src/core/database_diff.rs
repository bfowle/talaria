...
impl DatabaseDiffer {
    ...
    pub fn compare(&self, old_path: &Path, new_path: &Path) -> Result<ComparisonResult> {
        ...
        println!("Comparing {} vs {} sequences...", old_sequences.len(), new_sequences.len());
        ...
        for id in old_ids.intersection(&new_ids) {
            ...
            if self.headers_only {
                ...
                if old_seq.sequence != new_seq.sequence {
                    let similarity = self.calculate_similarity(&old_seq.sequence, &new_seq.sequence);
    ...
    fn load_database(&self, path: &Path) -> Result<HashMap<String, Sequence>> {
        let sequences_vec = parse_fasta(path)
    ...
    fn detect_changes(&self, old: &Sequence, new: &Sequence) -> Vec<SequenceChange> {
        ...
        if old.sequence.len() == new.sequence.len() {
            let mutations = old.sequence.iter()
...
impl ComparisonResult {
    fn new(old_path: std::path::PathBuf, new_path: std::path::PathBuf, old_count: usize, new_count: usize) -> Self {
        Self {
            old_path,
            new_path,
            old_count,
    ...
    fn calculate_statistics(&mut self, old_sequences: &HashMap<String, Sequence>, new_sequences: &HashMap<String, Sequence>) {
        ...
        let old_taxa: HashSet<u32> = old_sequences.values()
            .filter_map(|s| s.taxon_id)
            .collect();
        let new_taxa: HashSet<u32> = new_sequences.values()
            .filter_map(|s| s.taxon_id)
            .collect();
