...
/// CASG content addressing for workspace files
pub struct CasgWorkspaceManager {
    /// Root directory for CASG operations
    pub casg_root: PathBuf,
...
impl CasgWorkspaceManager {
    ...
    pub fn cleanup_cache(&self, max_age_days: u64) -> Result<usize> {
        let cache_dir = self.casg_root.join("cache");
        let mut removed = 0;
    ...
    /// Get CASG statistics
    pub fn get_statistics(&self) -> Result<CasgStatistics> {
        let mut stats = CasgStatistics::default();

        // Count temporal workspaces
        let temporal_dir = self.casg_root.join("temporal");
        if temporal_dir.exists() {
            stats.active_workspaces = fs::read_dir(&temporal_dir)?.count();
        }

        // Count preserved workspaces
        let preserved_dir = self.casg_root.join("preserved");
        ...
        stats.total_size_bytes = Self::calculate_dir_size(&self.casg_root)?;
...
impl CasgStatistics {
    pub fn print(&self) {
        println!("CASG System Statistics:");
...
/// Helper to ensure CASG operations are atomic
pub struct CasgTransaction {
    operations: Vec<CasgOperation>,
    rollback_operations: Vec<CasgOperation>,
}
...
impl CasgTransaction {
    ...
    pub fn write_file(&mut self, path: PathBuf, content: Vec<u8>) {
        self.operations.push(CasgOperation::WriteFile {
            path: path.clone(),
            content,
        });
    ...
    fn execute_operation(op: &CasgOperation) -> Result<()> {
        match op {
            CasgOperation::WriteFile { path, content } => {
                fs::write(path, content)?;
            }
            CasgOperation::DeleteFile { path } => {
                if path.exists() {
                    fs::remove_file(path)?;
                }
            }
            CasgOperation::CreateDir { path } => {
                fs::create_dir_all(path)?;
            }
            CasgOperation::DeleteDir { path } => {
                if path.exists() {
                    fs::remove_dir_all(path)?;
                }
            }
