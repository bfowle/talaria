use crate::casg::{CASGRepository, DiffResult, StandardTemporalManifestDiffer};
use crate::casg::differ::{TemporalManifestDiffer, DiffOptions, ChangeType};
...
pub fn run(args: DiffArgs) -> anyhow::Result<()> {
    ...
    // Load repositories
    let from_repo = CASGRepository::open(&from_path)?;
    let to_repo = CASGRepository::open(&to_path)?;
...
fn display_taxonomy_diff(_from: &CASGRepository, _to: &CASGRepository) -> anyhow::Result<()> {
    println!("\n{}", "─".repeat(60));
