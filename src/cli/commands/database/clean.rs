...
pub fn run(args: CleanArgs) -> anyhow::Result<()> {
    ...
    let mut repo = CASGRepository::open(&base_path)?;
...
fn clean_orphaned_chunks(repo: &mut CASGRepository, dry_run: bool) -> anyhow::Result<(usize, usize)> {
...
fn clean_duplicate_chunks(_repo: &mut CASGRepository, _dry_run: bool) -> anyhow::Result<(usize, usize)> {
...
fn compact_database(repo: &mut CASGRepository, dry_run: bool) -> anyhow::Result<usize> {
...
fn estimate_chunk_size(chunks: &[crate::casg::types::SHA256Hash]) -> usize {
