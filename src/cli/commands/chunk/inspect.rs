/// Inspect CASG chunk distribution and taxonomic organization
use anyhow::Result;
use clap::Args;
use colored::*;
...
use crate::casg::types::{ChunkMetadata, TaxonId, TemporalManifest};
...
impl ManifestInspector for TemporalManifest {
    ...
    fn inspect_chunk_distribution(&self) -> Result<String> {
        ...
        let taxonomy_path = crate::core::paths::talaria_databases_dir().join("taxonomy/current");
...
pub fn run(args: InspectArgs) -> Result<()> {
    // Parse database and version
    let (database, version) = parse_database_spec(&args.database)?;

    println!(
        "{}",
        format!("CASG Chunk Inspection: {}", args.database)
            .bold()
            .cyan()
    );
    println!("{}", "═".repeat(60));
    ...
    let base_path = crate::core::paths::talaria_databases_dir();
    ...
    match args.format {
        OutputFormat::Summary => {
            display_summary(&db_path, &database, version.as_deref())?;
        ...
        OutputFormat::Detailed => {
            display_detailed(&db_path, &database, version.as_deref(), true)?;
...
/// Parse database specification (database:version)
fn parse_database_spec(spec: &str) -> Result<(String, Option<String>)> {
    if let Some((db, ver)) = spec.split_once(':') {
        Ok((db.to_string(), Some(ver.to_string())))
    } else {
...
fn display_summary(db_path: &Path, database: &str, version: Option<&str>) -> Result<()> {
    ...
    println!("CASG Structure:");
    if manifest_path.exists() {
        println!("  {} Manifest found", "✓".green());
        let size = std::fs::metadata(&manifest_path)?.len();
        println!("    Size: {}", crate::utils::format::format_bytes(size));
    } else {
...
fn display_detailed(
    ...
    display_summary(db_path, database, version)?;
...
fn display_json(db_path: &Path, database: &str, version: Option<&str>) -> Result<()> {
    ...
    if manifest_path.exists() {
        ...
        output["manifest"] = json!({
            "exists": true,
            "size_bytes": size,
            "size": crate::utils::format::format_bytes(size),
        });
    }
    ...
    if chunks_dir.exists() {
        ...
        output["chunks"] = json!({
            ...
            "total_size": crate::utils::format::format_bytes(total_size),
        });
...
fn collect_chunk_files(dir: &Path, files: &mut Vec<(std::path::PathBuf, u64)>) -> Result<()> {
    for entry in std::fs::read_dir(dir)? {
        ...
        if path.is_dir() {
            ...
            .extension()
            .map_or(false, |ext| ext == "chunk" || ext == "zst")
        {
...
fn load_taxonomy_db() -> Result<TaxonomyDB> {
    ...
    let taxonomy_dir = crate::core::paths::talaria_databases_dir()
        .join("taxonomy")
        .join("current")
        .join("tree");
