...
pub struct SyncArgs {
    ...
    #[arg(long, default_value = "talaria-casg")]
    pub prefix: String,
...
pub fn run(args: SyncArgs) -> Result<()> {
    use crate::casg::cloud::{CloudConfig, CloudSyncManager, SyncDirection, SyncOptions, create_storage};

    // Store verbose flag before moving args
    let verbose = args.verbose();

    // Determine local CASG path
    ...
    if !local_path.exists() {
        anyhow::bail!("CASG repository not found at {}. Initialize it first with 'talaria casg init'", local_path.display());
    }
