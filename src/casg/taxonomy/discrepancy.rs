...
impl DiscrepancyDetector {
    ...
    pub fn detect_all(&self, storage: &CASGStorage) -> Result<Vec<TaxonomicDiscrepancy>> {
        let mut discrepancies = Vec::new();
    ...
    fn check_discrepancy(
        ...
        discrepancy_type.map(|dtype| TaxonomicDiscrepancy {
            sequence_id: sequence_id.to_string(),
            header_taxon: header_taxon.clone(),
            mapped_taxon: mapped_taxon.clone(),
        ...
        for disc in discrepancies {
            ...
            *by_type.entry(type_name).or_default() += 1;
            ...
            if let Some(ref taxon) = disc.header_taxon {
                affected_taxa.insert(taxon.clone());
            }
