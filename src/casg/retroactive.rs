...
impl RetroactiveAnalyzer {
    ...
    /// Create from CASGRepository
    pub fn from_repository(repository: CASGRepository) -> Self {
        Self {
    ...
    fn count_unique_taxa(&self, sequences: &[Sequence]) -> usize {
        let taxa: HashSet<_> = sequences
            .iter()
            .filter_map(|s| s.taxon_id.map(|t| TaxonId(t)))
            .collect();
        taxa.len()
    }
...
impl RetroactiveAnalyzer {
    ...
    fn map_event_type(&self, event_type: &crate::casg::temporal::TimelineEventType) -> EventType {
        use crate::casg::temporal::TimelineEventType;
