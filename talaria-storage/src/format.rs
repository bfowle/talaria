...
impl ManifestFormat for TalariaFormat {
    ...
    fn serialize_value(&self, value: &serde_json::Value) -> Result<Vec<u8>> {
        // First serialize to MessagePack with magic header
        let mut msgpack_data = Vec::with_capacity(TALARIA_MAGIC.len() + 1024 * 512);
        msgpack_data.extend_from_slice(TALARIA_MAGIC);
        msgpack_data.extend_from_slice(&rmp_serde::to_vec(value)?);
    ...
    fn deserialize_value(&self, data: &[u8]) -> Result<serde_json::Value> {
        // First decompress with Zstandard
        let decompressed = if data.len() >= 4 && data[0] == 0x28 && data[1] == 0xb5 {
        ...
        let content = if decompressed.starts_with(TALARIA_MAGIC) {
            &decompressed[TALARIA_MAGIC.len()..]
        } else {
            &decompressed[..]
        };
    ...
    fn compression_ratio(&self) -> f32 {
        0.05 // ~95% size reduction with MessagePack + Zstandard
    }
