...
use talaria_herald::download::workspace::{
...
async fn test_cleanup_on_success() -> Result<()> {
    ...
    let _files = talaria_herald::download::workspace::FileTracking::new();
...
async fn test_concurrent_download_prevention() -> Result<()> {
    ...
    let lock1 = talaria_herald::download::workspace::DownloadLock::try_acquire(&workspace)?;

    // Try to start second download - should fail
    let lock2 = talaria_herald::download::workspace::DownloadLock::try_acquire(&workspace);
    ...
    let lock3 = talaria_herald::download::workspace::DownloadLock::try_acquire(&workspace);
    assert!(
        lock3.is_ok(),
