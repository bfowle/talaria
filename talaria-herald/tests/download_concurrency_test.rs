...
async fn test_state_persistence_across_sessions() -> Result<()> {
    ...
    state1.transition_to(talaria_herald::download::workspace::Stage::Downloading {
        bytes_done: 1024,
        total_bytes: 2048,
    ...
    assert!(matches!(
        state2.stage,
        talaria_herald::download::workspace::Stage::Downloading {
            bytes_done: 1024,
            ..
        }
    ));
...
async fn test_workspace_cleanup_preserves_active() -> Result<()> {
    ...
    let cleaned = talaria_herald::download::workspace::cleanup_old_workspaces(1)?;
