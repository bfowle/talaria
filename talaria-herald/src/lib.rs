//! Content-Addressed Sequence Graph (HERALD) System
//!
...
pub use storage::{HeraldStorage, StorageChunkInfo, StorageStats};
pub use types::*;
...
impl HeraldRepository {
    ...
    pub fn init(base_path: &Path) -> Result<Self> {
        let storage = HeraldStorage::new(base_path)?;
    ...
    /// Open an existing HERALD repository
    pub fn open(base_path: &Path) -> Result<Self> {
        let storage = HeraldStorage::open(base_path)?;
        let manifest =
