...
//! # use std::path::PathBuf;
//! # fn main() -> Result<()> {
//! use talaria_herald::operations::{ReductionManager, ReductionManifest};
//! use talaria_herald::HeraldStorage;
//!
//! # let storage_path = PathBuf::from("/tmp/storage");
//! let storage = HeraldStorage::new(&storage_path)?;
//! let mut manager = ReductionManager::new(storage);
//! // Save and load reduction manifests
//! # use talaria_herald::operations::ReductionParameters;
