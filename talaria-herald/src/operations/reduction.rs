/// HERALD-based reduction manifest and management
///
...
pub struct ReductionStatistics {
    ...
    /// Deduplication ratio from HERALD
...
impl ReductionManifest {
    ...
    pub fn compute_merkle_roots(&mut self) -> Result<()> {
        ...
        if !self.reference_chunks.is_empty() {
            let ref_chunks: Vec<ManifestMetadata> = self
                .reference_chunks
                .iter()
        ...
        if !self.delta_chunks.is_empty() {
            let delta_chunks_meta: Vec<ManifestMetadata> = self
                .delta_chunks
                .iter()
                .map(|c| ManifestMetadata {
                    hash: c.chunk_hash,
...
/// Manager for reduction operations in HERALD
pub struct ReductionManager {
    storage: crate::HeraldStorage,
}

impl ReductionManager {
    pub fn new(storage: crate::HeraldStorage) -> Self {
        Self { storage }
    }
