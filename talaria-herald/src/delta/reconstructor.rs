...
use std::sync::Arc;
/// Delta reconstruction for HERALD delta chunks
///
...
impl DeltaReconstructor {
    ...
    pub fn reconstruct_chunk(
        &self,
        delta_chunk: &TemporalDeltaChunk,
    ...
    pub fn check_chain_depth(&self, delta_chunk: &TemporalDeltaChunk) -> usize {
    ...
    pub fn reconstruct_multiple(
        &self,
        delta_chunks: Vec<&TemporalDeltaChunk>,
...
impl DeltaChainManager {
    ...
    pub fn add_chunk(&mut self, chunk: &TemporalDeltaChunk) {
        if let ChunkClassification::Delta { reference_hash, .. } = &chunk.chunk_type {
            let depth = self
                .chain_map
                .get(reference_hash)
