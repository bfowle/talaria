...
/// Bi-temporal database functionality for HERALD
///
...
impl BiTemporalDatabase {
    ...
    pub fn new(storage: Arc<HeraldStorage>) -> Result<Self> {
...
impl DatabaseSnapshot {
    fn from_manifest(manifest: Manifest, storage: Arc<HeraldStorage>) -> Self {
    ...
    pub fn export_fasta(&self, path: &Path) -> Result<()> {
        ...
        if let Some(coord) = self.coordinate() {
            writeln!(file, "; HERALD Bi-Temporal Export")?;
            writeln!(
                file,
                "; Sequence Date: {}",
...
mod tests {
    ...
    fn test_bi_temporal_empty_database() -> Result<()> {
        ...
        let storage = Arc::new(HeraldStorage::new(&env.sequences_dir())?);
