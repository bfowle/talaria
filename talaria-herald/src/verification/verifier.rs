...
/// Cryptographic verification for HERALD
...
pub struct HeraldVerifier<'a> {
    storage: &'a HeraldStorage,
    manifest: &'a TemporalManifest,
}

impl<'a> HeraldVerifier<'a> {
    pub fn new(storage: &'a HeraldStorage, manifest: &'a TemporalManifest) -> Self {
    ...
    pub fn verify_consistency(&self) -> Result<ConsistencyReport> {
        ...
        for chunk_meta in &self.manifest.chunk_index {
            if !self.storage.has_chunk(&chunk_meta.hash) {
                issues.push(ConsistencyIssue::MissingChunk(chunk_meta.hash));
            }
        }
...
impl<'a> BatchVerifier<'a> {
    pub fn new(storage: &'a HeraldStorage, manifest: &'a TemporalManifest) -> Self {
        Self {
            verifier: HeraldVerifier::new(storage, manifest),
            parallel: false,
        }
    }
...
mod tests {
    ...
    fn setup_test_storage() -> (HeraldStorage, TempDir, TemporalManifest) {
