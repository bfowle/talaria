...
use crate::types::{ManifestMetadata, SHA256Hash, TemporalManifest};
...
impl StandardTemporalManifestValidator {
    ...
    fn check_overlaps(&self, _manifest: &TemporalManifest) -> Vec<ValidationError> {
        // ManifestMetadata doesn't have offset field in current implementation
        // Overlap checking would require different approach
        Vec::new()
    }

    async fn verify_chunk_storage(&self, chunk: &ManifestMetadata) -> Option<ValidationError> {
    ...
    async fn verify_chunk_hash(&self, chunk: &ManifestMetadata) -> Option<ValidationError> {
        let chunk_path = self.chunks_dir.join(chunk.hash.to_hex());
